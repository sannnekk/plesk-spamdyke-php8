...
class readlog extends paa
{
   ...
   function setSpamDykeConf()
   {
      ...
      $this->spamDykeConf["config-mysql-database"] = DB_NAME_SPAMDYKE;
      $this->spamDykeConf["log-level"] = LOG_LEVEL;

      if (!isset($this->spamDykeConf["log-level"]) || empty($this->spamDykeConf["log-level"])) {
         $this->spamDykeConfError = SCP_NO_LOG_LEVEL;
         $this->spamDykeInstalled = false;
      } else if (!isset($this->spamDykeConf["config-mysql-database"]) || empty($this->spamDykeConf["config-mysql-database"])) {
   ...
   function getAllBlocked()
   {
      ...
      $_allDomains = is_null($this->allDomains) ? "" : $this->allDomains;
      $_allDomains = gettype($_allDomains) == "string"
         ? array($_allDomains)
         : $this->allDomains;


      if (!$this->dieRechte->isAllView) {
         $searchStr = "AND ( `to` LIKE '%@" . implode("' OR `to` LIKE '%@", $_allDomains) . "' or ";
         $searchStr .= " `from` LIKE '%@" . implode("' OR `from` LIKE '%@", $_allDomains) . "')";
      }
      ...
      if ($this->anzVars["anz_Von"]) $dasLog = $this->anzVars["anz_Von"];
      $currentquery = "SELECT * FROM " . $this->spamDykeConf["config-mysql-database"] . ".spamdyke_log_table WHERE date_format(time,'%Y-%m-%d') = '" . $dasLog . "' " . $searchStr . " ORDER BY time DESC LIMIT 0," . (is_null($this->anzVars["anz_PerPage"]) || empty($this->anzVars["anz_PerPage"]) ? 100 : $this->anzVars["anz_PerPage"]);
      $res = mysqli_query($this->plesk_db_conn, $currentquery);
