...
// added to support php 8.0+ [by sannnekk] 
require('LegacyResolver.php');

$action = "main";

if ($_GET["action"] != "") $action = $_GET["action"];

$scp = scp::factory($_SESSION, $_GET, array(DB_HOST, DB_NAME, DB_USR, DB_PWD));
...
<head>
   ...
   <script type="text/javascript">
      ...
      function toggleStats(inFade, outFade) {
         ...
         $("#" + inFade).fadeIn("fast", function() {
            ...
            if (inFade == "topStat" && !stats_top_generated) {
               ...
               $("#topStat").load("topstat.php?<?php $scp->getQueryString(); ?>&todo=anzeige<?php echo ($scp->getAdmin()->isAllView) ? "&action=allview" : ""; ?>", {
                  anzeige: von,
                  land: jn
               }, function() {
      ...
      function storeValues() {
         ...
         vals = new Array("einstellungen_fe", "filter_fe"
            <?php echo ($scp->plesk_session->chkLevel(IS_ADMIN) || $scp->getAdmin()->checkResellerRight()) ? ',"rechte_fe"' : ''; ?>);
      ...
      function abuseReport(vals, email) {
         ...
         $.post("index.php?<?php $scp->getQueryString(); ?>&action=abusemail", {
            rec: email,
            from: from,
            to: to,
            ip: ip,
