...
class AdminRights
{
   ...
   var $settings_5_0_0 = array(
      "relay-level" => array("normal", "block-all", "allow-all"),
   ...
   function AdminRights($dom, $glDir, $xtea, $db)
   {
      ...
      if (is_null($db)) {
         $this->db_conn = LegacyResolver::reconnectDB();
      } else {
         echo "<h1>NOT NULL</h1>";
      }
      ...
      while ($file = readdir($dirlist)) {
         if ($file != '.' && $file != '..') {
            ...
            if (is_dir($newpath)) {
               ...
               var_dump($this->spamdykeFolderConfigDir);
   ...
   function setSpamDykeConf($conf, $post)
   {
      ...
      if (!is_null($post["dns-whitelist-entry"])) {
      ...
      if (!is_null($post["rhs-blacklist-entry"])) {
   ...
   function getOtherSettings($conf, $ver)
   {
      ...
      if (version_compare($treffer, "5.0.0", '>=')) {
         while (list($k, $v) = each($this->otherSettings[$ver] ?? $empt)) {
            while (list($key, $val) = each($conf)) {
               $found = "";
   ...
   function isSpamReport()
   {
      ...
      if (
         empty($this->abuseDetails["sender"]) ||
         empty($this->abuseDetails["titel"]) ||
         empty($this->abuseDetails["template"])
      ) return 0;
